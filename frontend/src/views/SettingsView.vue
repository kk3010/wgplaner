...
const setColor = (event: Event) => {
    userState.color = event.target?.value
}
...
<template>
    <div class="flex flex-col items-center w-full max-w-sm mx-auto space-y-4">
        ...
        <form @submit.prevent="updateUser({ firstName: userState.firstName })">
            <div class="form-control">
                ...
                <div class="relative">
                    <input id="firstName" type="text" v-model="userState.firstName" class="w-full pr-16 input input-primary input-bordered">
                    <button type="submit" class="absolute top-0 right-0 rounded-l-none btn btn-primary">Change</button>
                </div> 
            </div>
        </form>
        <form @submit.prevent="updateUser({ lastName: userState.lastName })">
            <div class="form-control">
                ...
                <div class="relative">
                    <input id="lastName" type="text" v-model="userState.lastName"  class="w-full pr-16 input input-primary input-bordered"> 
                    <button type="submit" class="absolute top-0 right-0 rounded-l-none btn btn-primary">Change</button>
                </div>
            </div> 
        </form>
        <form @submit.prevent="updateUser({ firstName: userState.firstName })">
            <div class="form-control">
                ...
                <div class="relative">
                    <input id="email" type="email" v-model="userState.email"  class="w-full pr-16 input input-primary input-bordered"> 
                    <button type="submit" class="absolute top-0 right-0 rounded-l-none btn btn-primary">Change</button>
                </div>
            </div> 
        </form>
    ...
    <div class="mt-12 space-y-8">
        <div class="divider">
            <h2 class="text-2xl">Danger Zone</h2>
        </div>
        <button @click="handleLeaveFlat" class="rounded block btn btn-outline btn-error mx-auto">Leave flat</button>
        <button @click="handleDeleteUser" class="rounded block btn btn-outline btn-error mx-auto">Delete User</button>
    </div>
</template>